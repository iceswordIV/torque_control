...
1. Use `computed_pid_friction_model`.
2. Test J1-J3 first.
3. If J1-J3 are stable, optionally test J1-J6.
4. If joint tests are stable, optionally test a small forward pose.
...
- Use torque limit `"5 8 10 10 3 3"` for first real tests.
...
In that fallback case, Python `--tau-limit` is the only clamp, so do not run large tests.

Watch the bridge output. It should print a rate. If the rate is very low or unstable, stop.

---

# Part A: separate first-hand tests for J1-J3
...
# Part B: create a J1-J3 sequence script

Only use this after the separate first-hand tests above are stable.
...
echo
echo "All J1-J3 small real tests completed."
SH
...
# Part C: optional J1-J6 small sequence

Only run this if J1-J3 are stable. This is still small-angle only.
...
cat > run_real_j1_to_j6_computed_pid_small.sh <<'SH'
#!/usr/bin/env bash
set -euo pipefail
...
run_one() {
  ...
  echo
  echo "============================================================"
  echo "REAL Z1 J1-J6 small computed_pid_friction_model test"
  echo "joint=${joint}, angle=${angle} deg"
  echo "log=logs/${label}.csv"
  echo "STOP if wrong direction, vibration, sagging, or unexpected motion."
  echo "============================================================"
...
echo "Real Z1 J1-J6 small computed-PID/friction sequence."
echo "Run this only after J1-J3 are stable."
echo "Positive J4 is excluded."
echo

run_one 1  5  real_j1_pos5_j1to6 8  8  25
run_one 2  5  real_j2_pos5_j1to6 10 10 28
run_one 3 -5  real_j3_neg5_j1to6 10 10 28
run_one 4 -2  real_j4_neg2_j1to6 10 10 28
run_one 5  5  real_j5_pos5_j1to6 8  8  25
run_one 6  5  real_j6_pos5_j1to6 8  8  25

echo
echo "All J1-J6 small real tests completed."
SH

chmod +x run_real_j1_to_j6_computed_pid_small.sh
```
...
```bash
cd /home/icesword/Desktop/torque_control/z1_project
bash run_real_j1_to_j6_computed_pid_small.sh
```

---

# Part D: optional small forward pose test

Only run this if J1-J6 small tests are stable.

Do **not** start with full forward pose `[0, 1.5, -1, -0.54, 0, 0]`.

Start with 10% scale:

```text
full target = [0, 1.5, -1.0, -0.54, 0, 0]
10% target = [0, 0.15, -0.10, -0.054, 0, 0]
```
...
python3 torque_main.py \
  ...
  --tau-limit "5 8 10 10 3 3" \
  --dynamics-mode analytic \
  --csv-log logs/real_forward_pose_10pct_computed_pid_first.csv
```

If 10% is stable, next possible target is 25%:

```text
25% target = [0, 0.375, -0.25, -0.135, 0, 0]
```
...
python3 torque_main.py \
  ...
  --tau-limit "5 8 10 10 3 3" \
  --dynamics-mode analytic \
  --csv-log logs/real_forward_pose_25pct_computed_pid_second.csv
```

Do not test 50%, 75%, or 100% forward pose until the small forward pose works.
